...
def collate_pool(dataset_list):
    ...
    batch_space_encode = []
    base_idx = 0
    for i, ((atom_fea, nbr_fea, nbr_fea_idx), target, cif_id, space_encode)\
        ...
        batch_space_encode.append(space_encode)
        base_idx += n_i
    ...
    return (torch.cat(batch_atom_fea, dim=0),
        ...
        batch_cif_ids, \
        torch.cat(batch_space_encode, dim=0)
...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        crystal = Structure.from_file(os.path.join(self.root_dir,
                                                   cif_id + '.cif'))
        atom_fea = [self.ari.get_atom_fea(crystal[i].specie.number)
                    # 获得晶体的元素，根据元素获得相应的编码值，根据编码值获得相应元素的初始向量，再把相应元素的向量进行叠加
                    for i in range(len(crystal))]

        crystal_frac_coord = torch.tensor(crystal.frac_coords, dtype=torch.float32)
        crystal_cart_coord = torch.tensor(crystal.cart_coords, dtype=torch.float32)
        crystal_site_coord = crystal.sites
        abc = []
        angles = []
        length = []
        for i in range(len(crystal)):
            abc.append(list(crystal_site_coord[i].lattice.abc))
            angles.append(crystal_site_coord[i].lattice.angles)
        abc = torch.tensor(abc, dtype=torch.float32)
        angles = torch.tensor(angles, dtype=torch.float32)
        angles = angles / 180 * 3.1415
        space_encode = torch.cat([crystal_frac_coord, crystal_cart_coord, abc, angles], dim=1)
        atom_fea = np.stack(atom_fea, axis=0)
        atom_fea = atom_fea[:, np.newaxis, :, :]
        ...
        return (atom_fea, nbr_fea, nbr_fea_idx), target, cif_id, space_encode
